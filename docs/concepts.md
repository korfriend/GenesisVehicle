...
- **High-level — `VehicleScene`** (the recommended entry point). It owns the
  ...
  the dual-scene optimization), and the registered vehicles / static / dynamic
...
|---|---|---|
| Owns | `gs.init` · scene(s) · `build` · `step` · raycast · dual-scene | you own the scene + step loop |
...
`obstacles_and_ramp` / `dual_scene_terrain` use `VehicleScene`; the L2 / L3
